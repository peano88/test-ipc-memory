...
func main() {
	...
	sendMsgs := func() {
		<-start
		for i := 0; i < nrMsgs; i++ {
			ipcmsg, err := prepareMsg(msg)
			if err != nil {
				log.Fatalf("Error while preparing message: %v index %d", err, i)
			}
			for {
				if err := msgsndPrepared(qid, len(msg.Mtext), ipcmsg, ipc.IPC_NOWAIT); err != nil {
